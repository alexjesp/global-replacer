...
function replaceGlobal (sourceRef, globals, from, to) {
  ...
  if (fromParts[0] !== toParts[0]) {
    ...
    var hiddenNodes = _.chain(globalNode.passedNodes).map(function (passedNode) {
      if (passedNode.node && passedNode.node.property.name === fromParts[1]) {
        return passedNode.node && passedNode.node.property;
      }
    }).compact().value();

    _.each(_.sortBy(properties.concat(hiddenNodes), 'start'), b(toParts[1]));
  }
